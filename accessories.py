...
class Notify:
    ...
    @staticmethod
    def start(username, sites) -> str:
        start(ascii_art, delay=0.1)
        if username or sites is not None:
            return f"[yellow][[bright_red]*[yellow][yellow]] [bright_blue]Searching {len(sites)} sites for target: [bright_yellow]{username}"

    # notify the user how many sites the username has been found
    @staticmethod
    def positive_res(username, count) -> str:
    ...
    @staticmethod
    def update(local_version, remote_version):
        return (
